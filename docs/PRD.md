...
Discord Commands are defined in `src/commands/chat`. Discord Interactions that are game-oriented (e.g., starting a game, submitting a turn) primarily call methods within the service layer (`src/services/*-service.ts`, such as `GameService`, `PlayerService`, etc.). These service methods encapsulate the core business logic and database interactions, returning structured data (e.g., game objects, status updates, error details) rather than pre-formatted strings.

The responsibility for constructing the complete, user-facing strings for all bot replies (DMs, public channel messages, ephemeral messages) lies within the command handler files in `src/commands/chat`. Each command file may contain dedicated functions or methods that take the structured data from service calls and format it into the appropriate message strings. This includes handling all presentation logic, such as formatting, emojis, markdown, and dynamic content based on game state or service responses.

Consequently, the command's `execute` method acts as a thin shell: it calls the necessary service method(s), receives structured data, passes this data to its local message generation function(s), and then sends the resulting string(s) to Discord. This approach decouples presentation logic from core service responsibilities and centralizes message formatting logic close to the command definition.

This architectural choice impacts testing: service layer tests will focus on the correctness and structure of the data returned, while unit tests for the message generation functions within command files will verify the exact string outputs. End-to-end command integration tests will continue to assert the final messages seen by the user. The service methods, in turn, depend on pure logic functions (e.g., in `src/utils/`) which are unit-tested without mocking external dependencies.
